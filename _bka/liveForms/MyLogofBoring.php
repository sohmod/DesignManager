...
if ($db = @mysql_connect (DB_HOST, DB_USER, DB_PASSWORD)) {
 ...
 if ($_SESSION['kp_s']){
 	...
 	$query = "DELETE FROM eb_fieldrecord WHERE idz='$id'";		
			...
			echo '<small>::Data '.$id.' not deleted</small>.<font color="red" size="-1"> mysql_error :-'.mysql_error().' -:</font>'; 	
			}  
endif; 		     
		}
?>
...
'<small>To view existing data, paste double semi-colons ( ;; ) in the input box,<br>
then click outside to activate register/ show button.</small>'; ?>			
