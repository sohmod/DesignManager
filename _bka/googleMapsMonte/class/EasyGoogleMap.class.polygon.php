...
/**----------------------------------------*/	
	 ...
	 * @return: void
	 */
	function SetMapWidth($width=750) {
	 ...
	 * @return:	void
	 */
	function SetMapHeight($height=550) {
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			$ret .= "if(GBrowserIsCompatible()  || !GBrowserIsCompatible()) { \n";
			...
			$ret .= "	var polygeocoder = new GClientGeocoder(); \n";
        ...
        foreach($this->_polygons as $_polygon) {
				...
				$ret .= "	var polyg_$i = {\n";
				$ret .= "	  infowindowtext: '".$_polygon['mylabel']."',\n";
				$ret .= "	  full: '".$_polygon['nglatlng']."'\n";
				$ret .= "	};\n\n";

				$ret .= "	polygon[$i] = polyg_$i.infowindowtext;\n\n";
				
				$ret .= "	polygeocoder.getLocations (\n";
				$ret .= "	  polyg_$i.full,\n";
				$ret .= "	  function($_polygon) {\n";
				$ret .= "		if(polyg_$i) {\n";
				$ret .= "		  _polygons[$i] = _polygon; \n";	
				$ret .= "		  map.setCenter(_polygon, {$this->mMapZoom});\n";
				//$ret .= "		  var polymarker = new GPolygon(parseFloat($_polygon['nglatlng']));\n";
				//$ret .= "		  GEvent.addListener(polymarker, 'click', function() {\n";
				//$ret .= "			polymarker.openInfoWindowHtml(polyg_$i.infowindowtext);\n";
				//$ret .= "		  });\n";
				//$ret .= "		  map.addOverlay(polyg_);\n";

				# show only info window to the first set address
				if ($i===0) 
					$ret .= "		  polymarker.openInfoWindowHtml(polyg_$i.infowindowtext);\n";
				$ret .= "		  pmarkers[$i] = polymarker;\n";
				$ret .= "		}\n";
				$ret .= "		else {\n";
				$ret .= "		  map.setCenter(new GLatLng(37.4419, -122.1419), {$this->mMapZoom});\n";
				$ret .= "		}\n";
				$ret .= "	  }\n";
				$ret .= "	); // end polygeocoder.getLatLng\n\n";
				// $ret .= "{$this->getPolygonJS()};\n";
				$i--;
			} # end for
			# end for polygon(s)
