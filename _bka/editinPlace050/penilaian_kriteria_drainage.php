...
<html>
<head>	
	<title>eggBlog - pt_drainage</title>
...
<br/>
<font color="gray">EditInPlace - <b>Penilaian</b> Teknikal Kerja Sivil : </font><font color="red">Drainage</font>
...
<span id="item_'.$scope.'#'.$chk_id.'">'.$item[200].'</span></td>
<td colspan="2" width="380" bgcolor="#FFFFFF" style="align:center;padding-left: 5px;font-size:12px">'.$kriteria[200].'</td>
...
$chk_d1="gred211".$j."#".$chk_id;$chk_d2="gred212".$j."#".$chk_id;;$chk_d3="gred213".$j."#".$chk_id;

$chk_c1="gred221".$j."#".$chk_id;$chk_c2="gred222".$j."#".$chk_id;  ///$chk_c3="gred223".$j."#".$chk_id;

$chk_p1="gred231".$j."#".$chk_id;$chk_p2="gred232".$j."#".$chk_id;$chk_p3="gred233".$j."#".$chk_id;$chk_p4="gred234".$j."#".$chk_id;

            $result=mysql_query("SELECT SUM(new_content) FROM kriteria where (id='$chk_d1' || id='$chk_d2' || id='$chk_d3')") or die ('Error performing query');
...
$sumgred2=$sumg[0]; // 1 el
            ...
            $result=mysql_query("SELECT SUM(new_content) FROM kriteria where (id='$chk_p1' || id='$chk_p2' || id='$chk_p3' || id='$chk_p4')") or die ('Error performing query');
$sumg=mysql_fetch_array($result);	  
$sumgred3=$sumg[0];  // 7 el
...
$sumskor1 = floatval($sumgred1)*floatval($wtg_1)/(3*5);
$sumskor2 = floatval($sumgred2)*floatval($wtg_2)/(2*5);
$sumskor3 = floatval($sumgred3)*floatval($wtg_3)/(4*5);

$sumgred = round($sumgred1+$sumgred2+$sumgred3,2);
$sumskor = round($sumskor1+$sumskor2+$sumskor3,2);

echo '<td bgcolor="#FDFECF" bordercolor="#EBEEF5" style="align:center;padding-left: 20px;font-size:11px">
<span id="gred20'.$j.'#'.$chk_id.'"><b>'. $sumgred .'</b></span></td>';
echo '<td bgcolor="#FDFECF" bordercolor="#EBEEF5" style="align:center;padding-left: 20px;font-size:11px">
<span id="skor20'.$j.'#'.$chk_id.'"><b>'.$sumskor.'</b></span></td>';		
InsertUpdata('skor20'.$j.'#'.$chk_id, $sumskor, $chk_id, $username, $kp ,$ajaxHost,$ajaxULogin,$ajaxPass,$ajaxDbase);
InsertUpdata('gred20'.$j.'#'.$chk_id, $sumgred, $chk_id, $username, $kp ,$ajaxHost,$ajaxULogin,$ajaxPass,$ajaxDbase);
...
$skor = floatval($gred)*floatval($weightage)/(3*5);
$skor = round($skor,2);
...
for ($j=1;$j<=$bilPetender;$j++){ 	
	echo " $('skor20".$j."#".$chk_id."').editInPlace({	form_type: 'textarea'});
	$('gred20".$j."#".$chk_id."').editInPlace({	form_type: 'textarea'});
