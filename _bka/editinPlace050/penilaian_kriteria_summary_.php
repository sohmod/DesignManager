...
$chk='UlasanPositif'.$j.'#'.$chk_id;
	$result=mysql_query("SELECT new_content FROM kriteria where id='$chk'") or die ('Error performing query');
    while($row=mysql_fetch_array($result, MYSQL_ASSOC)){
$UlasanPositifT = $row['new_content'];	}
echo '<td colspan="2" bgcolor="#FFFFFF" style="align:center;padding-left: 20px;font-size:12px">
<span id="UlasanPositif'.$j.'#'.$chk_id.'">'.$UlasanPositifT.'</span></td>';
$UlasanPositifT='';} echo '</tr>';
...
$chk='UlasanNegatif'.$j.'#'.$chk_id;
	$result=mysql_query("SELECT new_content FROM kriteria where id='$chk'") or die ('Error performing query');
    while($row=mysql_fetch_array($result, MYSQL_ASSOC)){
$UlasanNegatifT = $row['new_content'];	}
echo '<td colspan="2" bgcolor="#FFFFFF" style="align:center;padding-left: 20px;font-size:12px">
<span id="UlasanNegatif'.$j.'#'.$chk_id.'">'.$UlasanNegatifT.'</span></td>';
$UlasanNegatifT='';} echo '</tr>';
...
$chk='UlasanRumusan'.$j.'#'.$chk_id;
	$result=mysql_query("SELECT new_content FROM kriteria where id='$chk'") or die ('Error performing query');
    while($row=mysql_fetch_array($result, MYSQL_ASSOC)){
$UlasanRumusanT  = $row['new_content'];	}
echo '<td colspan="2" bgcolor="#FFFFFF" style="align:center;padding-left: 20px;font-size:12px">
<span id="UlasanRumusan'.$j.'#'.$chk_id.'">'.$UlasanRumusanT.'</span></td>';
$UlasanRumusanT='';} echo '</tr>';
