...
<html>
<head>	
	<title>eggBlog - BulkDrawings11</title>
...
$query = $dbf->query("SELECT tajuk,OIC FROM daftar_projek WHERE daftar_id='{$_SESSION['daftar_id']}'");
...
echo '<left><table width="800" border="1" cellspacing="" 
        cellpadding="" bordercolor="#E4F3CF" bgcolor="#FDFECF">
         <tr valign="top" width="800">
<small>'.$OIC.'\'s Job No: '.$_SESSION["daftar_id"].' List Of Drawings</small>
	    </tr></table></left>';
...
$db=mysql_connect('localhost','ajaxuser','practical') or die('Error connecting to the server');
mysql_select_db('ajax') or die('Error selecting database');
    $result=mysql_query("SELECT * FROM mydrawings WHERE daftar_id='{$_SESSION['daftar_id']}' ORDER BY regist_date DESC") or die ('Error performing query');
...
print "<left><table width=\"800\" border=\"1\" cellspacing=\"1\" cellpadding=\"0\" bordercolor=\"#E4F3CF\" bgcolor=\"#ffffff\" >
		...
		<td width=\"250\">
            <font color=\"#000000\" size=\"1\">".$nombor." &nbsp;Trk: ".$tarikh."</font>
            </td>
		<td width=\"50\">
              <font color=\"#000000\" size=\"1\">".$kategori."</font>
            </td>		
