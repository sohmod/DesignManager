...
// Get tentative or actual date
	...
	if (mysqli_num_rows($r) == 1) {
		list($dbevtdate) = mysqli_fetch_array($r, MYSQLI_NUM);}
...
// Get previous saved post_data to append
	...
	$r = mysqli_query($db, $q);
	if (mysqli_num_rows($r) == 1) {
		list($dbpost_data) = mysqli_fetch_array($r, MYSQLI_NUM);}
...
if ( $id_split == 'kemajuan2' ) {
			$query11 = $db->query("INSERT into laporanbulananRM11 
											(id_daftar,{$datacol},
											username,ppk,regist_date)
											values ('$chk_id','$new_value','$username','$ppk',NOW())");
											
			$query22 = $db->query("UPDATE laporanbulananRM11 SET 
											{$datacol}='$new_value',
											username='$username',
											ppk='$ppk',
											regist_date=NOW() 
