...
if ($programEV=="UMUM" && ($OICEV<>"PENGARAH" || $lokasiEV<>"MALAYSIA" || $mod_kerjaEV<>"Umum" ) ) 
{die ("<center>Fail am/umum untuk Pengarah: Lokasi MALAYSIA & Modus Umum saja.</center>");} 

if ($programEV<>"UMUM" && ($OICEV=="PENGARAH" || $mod_kerjaEV=="Umum" ) ) 
...
OIC,startDate,siapDate,kp_s,registration_date,program,catatan,nosiri_fail,KemKlient) VALUES 
('$username','$tajuk','$lokasi','$mod_kerja','$OIC','$startDate','$siapDate','$kp_s',NOW(),
'$program','$catatan','$nosiri_fail','$KemKlient')";
...
<?php 
for ($Vlokasi=0;$Vlokasi <=16;$Vlokasi++) 
{ echo "<option>$Vlokasitxt[$Vlokasi]</option>\n";} ?> </select> 

Modus:<select name="mod_kerjaEV" id="mod_kerjaEV"> 
<?php
$Vmod_kerjatxt = array (0=>'Inhouse',1=>'Perunding',2=>'Perunding DB',3=>'Umum');
...
$oic_FAIL[$myrow[OIC]].$KOD_KEM[$myrow[KemKlient]].'/'.$KOD_NEG[$myrow[lokasi]].$myrow[nosiri_fail].'</font>
</div>'. 
'<div style="margin-top:-12px" > 
...
<!----------------------------------------------------DateInput------------------------------------------------------------->
<tr> <td colspan="2"> 
<br/><small>Kementerian Pelangan:</small><br><h><?php echo '<font color="gray">'.$myrow[KemKlient].'</font>';?>
</h></td></tr>
...
<tr><td colspan="2" style="margin-top:-200px"><small> 
NOMBOR FAIL: &nbsp;&nbsp;<font color="red"><b>
<?php 
if ($_SESSION['IDpendaftar']===$_SESSION['kp_s']){ 

echo $oic_FAIL[$myrow[OIC]].$KOD_KEM[$myrow[KemKlient]].'/'.$KOD_NEG[$myrow[lokasi]];?>
<input name="nosiri_failEV" type="text" id="nosiri_failEV" value="<?php echo $myrow[nosiri_fail] ?>"> 
<?php } else {echo $oic_FAIL[$myrow[OIC]].$KOD_KEM[$myrow[KemKlient]].'/'.$KOD_NEG[$myrow[lokasi]].$myrow[nosiri_fail] ?> 
<input name="nosiri_failEV" type="hidden" id="nosiri_failEV" value="<?php echo $myrow[nosiri_fail] ?>">
<?php } ?>
</b></font></td></tr>

<tr><td align="right">
CATATAN:</td> <td align="left"><small>
...
if ($Vprogram=="UMUM" && ($VOIC<>"PENGARAH" || $Vlokasi<>"MALAYSIA" || $Vmod_kerja<>"Umum" ) ) 
{die ("<center>Fail am/umum untuk Pengarah: Lokasi MALAYSIA & Modus Umum saja.</center>");} 

if ($Vprogram<>"UMUM" && ($VOIC=="PENGARAH" || $Vmod_kerja=="Umum" ) ) 
{die ("<center>Sila asain projek kepada KPPK, tidak PENGARAH &amp; Lokasi NEGERI2.</center>");} 
...
catatan='$catatan',nosiri_fail='$nosiri_fail',KemKlient='$KemKlient' WHERE daftar_id='$daftar_id';";
$result=mysql_query($sql); 
if (!$result) {
