...
require_once ('mysql_connect.php'); // Connect to the database.
		$q = "select tajuk, program AS ppk, mod_kerja, OIC, kp_s AS IDpendaftar from daftar_projek 
			where daftar_id = {$_GET['eid']} ";
		$r = mysql_query ($q);
		while ($tna = mysql_fetch_array ($r)) {
		$_SESSION['tajuk'] = $tna[0];
		$_SESSION['ppk'] = $tna[1];  // program
		$_SESSION['mod_kerja'] = $tna[2];
		$_SESSION['OIC'] = $tna[3];
		$_SESSION['IDpendaftar'] = $tna[4];
...
echo ' | <a href="../_bka/borehole/index.php" style="text-decoration:none" >Borehole</a>' ;}
?>
</span></div>
</table>
</div>
...
echo '<br>line 164--programEV='.$programEV.'--OICEV='.$OICEV.'--lokasiEV='.$lokasiEV.'--mod_kerjaEV='.$mod_kerjaEV ;

echo '<br>line 166--SSdaftar_id='.$_SESSION['daftar_id'].'--SStajuk='.$_SESSION['tajuk'].'--SSmod_kerja='.$_SESSION['mod_kerja'].'--SSppk/program='.$_SESSION['ppk'].'--OIC='.$_SESSION['OIC'].'--SSIDpendaftar='.$_SESSION['IDpendaftar'].'<br>--SSno_bpks='.$_SESSION['no_bpks'].'--SSskop_kerja='.$_SESSION['skop_kerja'];
...
if ($programEV<>"BKA" && ($OICEV=="PENGARAH" || $mod_kerjaEV=="Piawai" ) ) 
...
$Vno_bpks= $no_bpksEV;
$_SESSION['ppk']=$Vprogram; //new

//ECHO $VOIC.'<--$VOIC' ;
if ($_SESSION['mod_kerja']<>'Piawai'){
$count = mysql_query("SELECT COUNT(*) FROM daftar_projek WHERE program ='$Vprogram' AND lokasi ='$Vlokasi' AND KemKlient ='$VKemKlient' ");}
if ($_SESSION['kp_s']==SUPERUSER_ID && $_SESSION['mod_kerja']=='Piawai'){
$count = mysql_query("SELECT COUNT(*) FROM daftar_projek WHERE mod_kerja ='Piawai' AND lokasi ='$Vlokasi' AND KemKlient ='$VKemKlient' ");}
echo '<br>SESSION kp_s 208:: '.$_SESSION['kp_s'];
echo '<br>SESSION ppk (program) 209:: '.$_SESSION['ppk'];
$count = mysql_fetch_array($count);
$Vnosiri_fail = $count[0]+1;
echo '<br>nosiri_fail after count  212:: '.$Vnosiri_fail;
...
<?php echo $oic_FAIL[$OIC].$KOD_KEM[$KemKlient].'/'.$KOD_NEG[$lokasi].$nosiri_fail ?>  
</td> </tr>
...
if (!$result) {
die('<center>Data tidak dikemaskini :  ' . mysql_error().'<br/><font color="gray"><small>Fail nombor yang diketik ('.$nosiri_fail.') telah ada, sila pinda nombor fail sediada mengikut turutan <br/>atau guna nom. dummi untuk fail sediada, 
...
<?php
$Vprogram= $_SESSION['ppk'];
$programEV=$Vprogram;
for ($Vprogram=0; $Vprogram<=6; $Vprogram++) 
{ echo "<option";
if ($programtxt[$Vprogram]==$programEV){ echo " SELECTED";} 
echo ">$programtxt[$Vprogram]</option>\n";} ?> </select>
...
<?php
$Vmod_kerjatxt = array (0=>'Inhouse',1=>'Perunding',2=>'Perunding DB',3=>'Piawai');
for ($Vmod_kerja=0; $Vmod_kerja <=3; $Vmod_kerja++) 
...
{ echo "<option>$VOICtxt[$VOIC]</option>\n";} ?> </select> 
<?php }
///////$_SESSION['ppk'] = $OICEV;
 ?>
...
<!--------------------------------------------------------------------------------------------------------------------->
<?php } 
$_SESSION['OIC'] = $OICEV;$_SESSION['program'] = $programEV;
echo '<br>SESSION OIC-ev 479:: '.$OICEV;
echo '<br>SESSION program-ev 480:: '.$programEV;
...
if (isset($_SESSION['first_name']) && $userON=='true') 
{$OIC =user_kppk($_SESSION['kp_s']);
           $_SESSION['ppk']=$programEV;       //$_SESSION['ppk']=$OIC;

$chkOIC = $_SESSION['OIC'];  //$OICEV;        //$chkOIC = $_SESSION['ppk'];
$chkKP_S = $_SESSION['IDpendaftar'];   //$_SESSION['kp_s'];
...
$result=mysql_query( "SELECT * FROM daftar_projek WHERE OIC = '$chkOIC' AND kp_s = '$chkKP_S' ORDER BY registration_date DESC",$db);
			}
				else { 
...
<font color="darkkhaki" size="1em"  style="margin-left:5px">Prog.&nbsp;:</font>
...
<font color="darkkhaki" size="1em"  style="margin-left:200px">No. fail&nbsp;:</font>
<font color="darkgreen" size="1em"  style="margin-left:5px">'.
$programFAIL[$myrow[program]].$KOD_KEM[$myrow[KemKlient]].'/'.$KOD_NEG[$myrow[lokasi]].$myrow[nosiri_fail].'</font>
</div>'. 
'<div style="margin-top:-12px" > 
<font color="darkkhaki" size="1em" style="margin-left:500px"></font>
<font color="darkgreen" size="0.2em" style="margin-left:5px" >'.$myrow[mod_kerja].'</font>
</div>'; ?>
</td>
</tr> 
...
'<div style="margin-top:-12px" > 
<font color="darkkhaki" size="1em"  style="margin-left:250px">No. fail&nbsp;:</font>
<font color="darkgreen" size="1em"  style="margin-left:5px">'.
$programFAIL[$myrow[program]].$KOD_KEM[$myrow[KemKlient]].'/'.$KOD_NEG[$myrow[lokasi]].$myrow[nosiri_fail].'</font>
</div>'. 
'<div style="margin-top:-12px" > 
<font color="darkkhaki" size="1em" style="margin-left:525px"></font>
<font color="darkgreen" size="0.2em" style="margin-left:5px" >'.$myrow[mod_kerja].'</font>
</div>'; ?>
</td>
</tr> 
...
<?php } else {echo $programFAIL[$myrow[program]].$KOD_KEM[$myrow[KemKlient]].'/'.$KOD_NEG[$myrow[lokasi]].'/'.$KOD_NEG[$myrow[lokasi]].$myrow[nosiri_fail] ?> 
...
NOMBOR BPKS:<input type="text" name="no_bpksEV" id="no_bpksEV"  value="<?php echo $myrow[no_bpks].'">';?>

<?php if ($_SESSION['kp_s']==SUPERUSER_ID){ ?>
&nbsp; MODUS:
...
<center><small>[ 
<a href="<?php echo "$fileName?del=$myrow[daftar_id]";?>" onCLick="return confirmDelete();"><small>Padam Data</small></a> ] </small>
<br> <br> </center> 
<?php } 
...
if ($Vprogram<>"BKA" && ($VOIC=="PENGARAH" || $Vmod_kerja=="Piawai" ) ) 
{die ("<center>Sila asain projek kepada KPPK, tidak PENGARAH &amp; Lokasi NEGERI2.</center>");} 
...
<?php echo $oic_FAIL[$VOIC].$KOD_KEM[$VKemKlient].'/'.$KOD_NEG[$Vlokasi].$Vnosiri_fail ?>  
